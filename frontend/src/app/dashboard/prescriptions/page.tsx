...
function formatEyeSummary(prefix: 'od' | 'os', row: OpticalPrescription): string {
    ...
    if (typeof axis === 'number' && !Number.isNaN(axis)) parts.push(`x ${axis}°`);
    ...
    const statCards = useMemo(() => ([
        {
            ...
            cardClassName: 'border-sky-100 bg-gradient-to-br from-sky-50/70 via-white to-white',
        ...
        },
        {
            ...
            cardClassName: 'border-blue-100 bg-gradient-to-br from-blue-50/70 via-white to-white',
        ...
        },
        {
            ...
            cardClassName: 'border-rose-100 bg-gradient-to-br from-rose-50/70 via-white to-white',
    ...
    return (
        <div className="w-full min-w-0 p-4 sm:p-5 md:p-6 lg:p-8 space-y-6">
            <div className="flex items-start justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900">Optical Prescriptions</h1>
                    <p className="mt-1 text-sm text-slate-500">Manage optical prescriptions</p>
                </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
                    <Select value={status} onValueChange={setStatus}>
                        <SelectTrigger className="h-10 w-full sm:w-[160px] border-slate-200">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {statCards.map(({ label, value, icon: Icon, valueClassName, iconClassName, iconWrapClassName, cardClassName, labelClassName }) => (
                    <div key={label} className={`rounded-xl border p-4 sm:p-5 shadow-sm ${cardClassName}`}>
                    ...
                    <TableHeader>
                        <TableRow className="bg-slate-50/80 hover:bg-slate-50/80">
                            {['PATIENT', 'TYPE', 'OD (RIGHT)', 'OS (LEFT)', 'DATE', 'EXPIRY', 'STATUS', 'ACTIONS'].map((h) => (
                                <TableHead key={h} className="text-[11px] font-bold text-slate-500 uppercase tracking-wider py-3 px-4 whitespace-nowrap">
                                    {h}
                                </TableHead>
                            ))}
                        </TableRow>
                                    ...
                                    <TableRow key={row.id} className="border-slate-100 hover:bg-slate-50/60">
                                        <TableCell className="py-4 px-4">
                                            <div className="min-w-[180px]">
                                                ...
                                                <p className="text-xs text-slate-500">ID: {patientDisplayId}</p>
                                            </div>
                                        </TableCell>
                                        <TableCell className="py-4 px-4 text-sm text-slate-700">{toTypeLabel(row.type)}</TableCell>
                                        <TableCell className="py-4 px-4 text-sm text-slate-700">{formatEyeSummary('od', row) || '-'}</TableCell>
                                        <TableCell className="py-4 px-4 text-sm text-slate-700">{formatEyeSummary('os', row) || '-'}</TableCell>
                                        <TableCell className="py-4 px-4 text-sm text-slate-700 whitespace-nowrap">{formatDate(row.createdAt)}</TableCell>
                                        <TableCell className="py-4 px-4 text-sm text-slate-700 whitespace-nowrap">{formatDate(row.expiryDate)}</TableCell>
                                        <TableCell className="py-4 px-4">
                                            <Badge
                                                variant="secondary"
                                                className={
