...
export default function SurgeriesPage() {
    ...
    const fetchRows = useCallback(async (searchTerm = '', status = 'all', date = '') => {
        setLoading(true);
        try {
            ...
            setRows(res.data as SurgeryRow[]);
        } catch (error: unknown) {
    ...
    const fetchAllSurgeries = useCallback(async () => {
        try {
            ...
            const list = (res.data || []) as SurgeryRow[];
    ...
    const fetchDoctors = useCallback(async () => {
        try {
            ...
            const list = (res.data || []) as Array<{ doctorId?: unknown; fullName?: unknown; branchId?: unknown }>;
    ...
    useEffect(() => {
        const timer = setTimeout(() => fetchRows(search, statusFilter, dateFilter), 300);
        return () => clearTimeout(timer);
    }, [search, statusFilter, dateFilter, fetchRows]);
        ...
        <div className="w-full min-w-0 p-4 sm:p-5 md:p-6 lg:p-8 space-y-6 animate-in fade-in duration-300">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-x-auto min-w-0">
                <DataTable
                    ...
                    data={filtered}
                    loading={loading}
                    onRefresh={() => fetchRows(search, statusFilter, dateFilter)}
                    itemLabel="surgeries"
                    hideSearch
                />
            </div>
