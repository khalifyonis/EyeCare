...
type TxRow = {
    ...
    opticalItem?: { id: string; itemName: string } | null;
    performedBy?: { fullName: string } | null;
};

export default function OpticalPurchasesPage() {
    ...
    const loadPurchases = () => {
        setLoading(true);
        api.get('/inventory/optical/transactions?type=IN&limit=100')
    ...
    const columns = useMemo<ColumnDef<TxRow>[]>(
        () => [
            ...
            {
                accessorKey: 'opticalItem.itemName',
                header: 'Item',
                cell: ({ row }) => row.original.opticalItem?.itemName ?? '—',
            },
            {
    ...
    return (
        <div className="w-full min-w-0 p-4 sm:p-5 md:p-6 lg:p-8 space-y-4">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
                Record stock received from suppliers. Click Receive stock, select an item, quantity, and unit price. New purchases appear in the table below.
            </p>
            <DataTable
                columns={columns}
                data={rows}
