...
type TxRow = {
    ...
    pharmacyItem?: { id: string; itemName: string; category?: string | null } | null;
...
export default function PharmacyTransactionsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (to) params.to = to;
            const res = await api.get('/inventory/pharmacy/transactions', { params });
            const body = res.data;
                        ...
                        <tbody>
                            ...
                            {!loading && rows.map(r => (
                                <tr key={r.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/30">
                                    ...
                                    <td className="px-4 py-2.5 font-medium text-slate-900 dark:text-slate-100">{r.pharmacyItem?.itemName || '—'}</td>
