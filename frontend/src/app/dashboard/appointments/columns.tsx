...
export const getAppointmentColumns = ({
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			return (
				<Link href={`/dashboard/patients/${patient.id}`} className="flex items-center gap-3.5 py-1.5 group cursor-pointer hover:bg-slate-50/50 dark:hover:bg-slate-800/20 -ml-2 pl-2 rounded-xl transition-all">
					...
					<div className="flex flex-col min-w-0">
						...
						<div className="flex items-center gap-2 mt-0.5">
							<span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 uppercase tracking-widest">#{String(patient.id).slice(0, 8)}</span>
							{patient.phone && <span className="text-[11px] font-medium text-slate-500">{patient.phone}</span>}
						</div>
					</div>
				</Link>
			);
