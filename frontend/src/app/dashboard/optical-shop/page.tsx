  ...
  return (
    <div className="optical-page w-full min-w-0 p-4 sm:p-6 lg:p-7 space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">Optical Shop</h1>
          <p className="mt-1 text-xl text-slate-600">Manage optical inventory and orders</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        ...
        <div className="divide-y divide-slate-200">
          {filtered.length === 0 ? (
            <div className="py-12 text-center text-lg text-slate-500">{loading ? 'Loading...' : 'No orders found'}</div>
          ) : (
            filtered.map((order) => {
              ...
              return (
                ...
                >
                  <div className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-cyan-100 text-cyan-700">
                    <Glasses className="h-7 w-7" />
                  </div>

                  <div className="min-w-0 flex-1">
                    <div className="text-xl md:text-2xl font-bold leading-none text-slate-900">OPT-{compactId(order.id)}</div>
                    <div className="mt-1 text-base md:text-lg text-slate-700 truncate">{order.patient?.fullName || 'Unknown Patient'}</div>
                  </div>

                  <div className="shrink-0 text-right">
                    {statusBadge(displayStatus)}
                    <div className="mt-2 text-xl md:text-2xl font-bold leading-none text-slate-900">{money(amount)}</div>
                    <div className="mt-1 text-sm md:text-base text-slate-500">{order.createdAt ? new Date(order.createdAt).toLocaleDateString() : '-'}</div>
                  </div>
      ...
      <div className="flex items-center gap-2">
        <Button variant="outline" onClick={load} disabled={loading} className="h-10 rounded-lg">
          Refresh
        </Button>
        <span className="text-sm text-slate-500">Total Orders: {backendStats.total}</span>
      </div>
    </div>
  )
}
