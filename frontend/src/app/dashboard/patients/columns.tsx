    ...
    const base: ColumnDef<PatientRow>[] = [
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <Link href={`/dashboard/patients/${p.id}`} className="flex items-center gap-3 group min-w-[160px]">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[13px] font-semibold text-slate-800 dark:text-white group-hover:text-[#0EA5E9] transition-colors leading-tight">
                                {name}
                            </span>
                            <span className="text-[11px] text-slate-400 font-medium mt-0.5">ID: {pid}</span>
                        </div>
                    </Link>
                );
            },
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex flex-col gap-0.5 min-w-[150px]">
                        ...
                        <span className="text-[12px] text-slate-500 font-medium">{p.phone || '—'}</span>
                    </div>
                );
            },
        },
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <span className={`text-[12px] font-medium ${doc ? 'text-slate-700 dark:text-slate-300' : 'text-slate-400'}`}>
                        {doc || 'No'}
                    </span>
                );
            },
            ...
            return (
                <div className="flex items-center gap-1">
                    <Link
                        ...
                        className="text-[12px] font-semibold text-[#0EA5E9] hover:text-[#0c96d4] transition-colors px-1"
                    >
                        View
                    </Link>
                    {canManage && (
                        <>
                            ...
                            <button
                                onClick={() => onEdit(p)}
                                className="text-[12px] font-semibold text-slate-600 dark:text-slate-300 hover:text-[#0EA5E9] transition-colors px-1"
                            >
                                Edit
                            </button>
                            <DropdownMenu>
